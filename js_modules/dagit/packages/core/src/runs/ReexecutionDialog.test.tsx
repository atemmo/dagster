...
describe('ReexecutionDialog', () => {
  ...
  it('prompts the user with the number of runs to re-execute', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/3 runs will be re\-executed\. do you wish to continue\?/i),
      ).toBeVisible();
    });
  });
  ...
  it('displays workspace mismatch errors', async () => {
    const mocks = {
      Run: () => ({
        // Pipeline does not exist in workspace, therefore cannot be re-executed
    ...
    await waitFor(() => {
      expect(screen.getAllByText(/Job not found in workspace/i)).toHaveLength(3);
    });
  });
